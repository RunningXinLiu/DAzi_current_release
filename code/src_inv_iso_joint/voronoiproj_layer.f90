...
!      integer iw(leniw)
      ...
      real depz(nz)
      real rw(lenrw)
      integer ncells,acells,ncell,athresh
      real dv(*),dres(*)
      ...
      call slarnv(1,iseed,nzrand,radnz)
      radnz = radnz*depz(nz-1)

      do ii = 1,nzrand
      do jj = 1,ncell
      !rrad((ii-1)*ncell+jj) = radius - (depz((ii-1)*2+1)+depz(ii*2))/2.0*hvratio
      rrad((ii-1)*ncell+jj) = radius - radnz(ii)*hvratio!(depz((ii-1)*2+1)+depz(ii*2))/2.0*hvratio
      enddo
      enddo
