...
subroutine CalSurfGAnisoJoint(nx, ny, nz, nparpi, vels, iw, rw, col, dsurf, &
    ...
    integer env_status
    real*8 timer_all_start, timer_all_end
    real*8 timer_tmp_start, timer_tmp_end
    real*8 timer_depth_ti, timer_depth_fd, timer_trace_total
    real*8 timer_travel, timer_srtimes, timer_rpaths, timer_row_assembly
    ...
    !--------------------------------------------------------------------------!
    timer_all_start = omp_get_wtime()
    timer_depth_ti = 0.0d0
    timer_depth_fd = 0.0d0
    timer_trace_total = 0.0d0
    timer_travel = 0.0d0
    timer_srtimes = 0.0d0
    timer_rpaths = 0.0d0
    timer_row_assembly = 0.0d0
    gdx=5
    gdz=5
    asgr=1
    sgdl=8
    ...
    timer_depth_ti = timer_tmp_end - timer_tmp_start
    ...
    timer_tmp_end = omp_get_wtime()
    timer_depth_fd = timer_tmp_end - timer_tmp_start
    ...
    do srcnum=1, nsrcsurf1(knumi)
        ...
        timer_tmp_end = omp_get_wtime()
        call travel(x,z,urg)
        timer_travel = timer_travel + (omp_get_wtime() - timer_tmp_end)
        !
        ...
        do istep = 1, nrc1(srcnum, knumi)
            ...
            timer_srtimes = timer_srtimes + (omp_get_wtime() - timer_tmp_end)
            count1 = count1 + 1
            dsurf(count1) = cbst1
            ...
            timer_rpaths = timer_rpaths + (omp_get_wtime() - timer_tmp_end)
            ...
            ! Keep the dense matrices synchronized with the sparse row assembly.
            ! This avoids reusing stale coe_a / coe_rho values in later diagnostics.
            ...
            enddo
            timer_row_assembly = timer_row_assembly + (omp_get_wtime() - timer_tmp_end)
            !------------------------------------------------------------------!
            ! For test
    ...
    timer_trace_total = omp_get_wtime() - timer_tmp_start
    ...
    timer_all_end = omp_get_wtime()
    write(6,'(a)') '  CalSurfGAnisoJoint timing breakdown:'
    write(6,'(a,f12.3,a)') '    depthkernelTI anisotropic kernel = ', timer_depth_ti, ' s'
    write(6,'(a,f12.3,a)') '    depthkernel backend = ', timer_depth_fd, ' s'
    write(6,'(a,f12.3,a)') '    trace/G loop total = ', timer_trace_total, ' s'
    write(6,'(a,f12.3,a)') '      travel/FMM only = ', timer_travel, ' s'
    write(6,'(a,f12.3,a)') '      srtimes interpolation = ', timer_srtimes, ' s'
    write(6,'(a,f12.3,a)') '      rpathsAzim = ', timer_rpaths, ' s'
    write(6,'(a,f12.3,a)') '      G row assembly = ', timer_row_assembly, ' s'
    write(6,'(a,f12.3,a)') '    CalSurfGAnisoJoint total = ', timer_all_end - timer_all_start, ' s'
    write(66,'(a)') '  CalSurfGAnisoJoint timing breakdown:'
    write(66,'(a,f12.3,a)') '    depthkernelTI anisotropic kernel = ', timer_depth_ti, ' s'
    write(66,'(a,f12.3,a)') '    depthkernel backend = ', timer_depth_fd, ' s'
    write(66,'(a,f12.3,a)') '    trace/G loop total = ', timer_trace_total, ' s'
    write(66,'(a,f12.3,a)') '      travel/FMM only = ', timer_travel, ' s'
    write(66,'(a,f12.3,a)') '      srtimes interpolation = ', timer_srtimes, ' s'
    write(66,'(a,f12.3,a)') '      rpathsAzim = ', timer_rpaths, ' s'
    write(66,'(a,f12.3,a)') '      G row assembly = ', timer_row_assembly, ' s'
    write(66,'(a,f12.3,a)') '    CalSurfGAnisoJoint total = ', timer_all_end - timer_all_start, ' s'
