...
module lsmrModule
  ...
  use  lsmrblasInterface, only : dnrm2 => dnrm2_dp, dscal => dscal_dp
  implicit none
  private
  public   :: LSMR
...
!                     x, istop, itn, normA, condA, normr, normAr, normx )
   subroutine LSMR  ( m, n, leniw, lenrw,iw,rw, b, damp,               &
    ...
    integer(kind=8),  intent(in) :: leniw
    integer(kind=8),  intent(in) :: lenrw
    ...
    interface
        subroutine aprod_dp(mode,m,n,x,y,leniw,lenrw,iw,rw)       ! y := y + A*x
         use lsmrDataModule, only : dp
...
!       end subroutine Aprod2
    ...
    !   call Aprod2(m, n, v, u)          ! v = A'*u
        call aprod_dp(2,m,n,v,u,leniw,lenrw,iw,rw)
       alpha = dnrm2 (n, v, 1)
    end if
    ...
    do
      ...
      ! call Aprod1(m, n, v, u)             ! u = A*v
        call aprod_dp ( 1,m,n,v,u,leniw,lenrw,iw,rw )      
       beta   = dnrm2 (m, u, 1)
