      subroutine depthkernel(nx,ny,nz,vel,pvRc,sen_vsRc,sen_vpRc,sen_rhoRc, &
        ...
        real vsz(nz)
        real*8 pv_col(kmaxRc)
        real*8 col_sen_vs(kmaxRc,nz), col_sen_vp(kmaxRc,nz), col_sen_rho(kmaxRc,nz)
	      integer mmax
        integer ii,jj,i,nn
	real dlnVs,dlnVp,dlnrho
        character(len=64) backend
        character(len=64) pdsurf_engine
        ...
        backend = ''
        call get_environment_variable('DAZI_DEPTHKERNEL_BACKEND', backend, status=env_status)
        if (env_status /= 0 .or. len_trim(backend) == 0) backend = 'default_fortran_fd'
        pdsurf_engine = 'default'
        if (trim(adjustl(backend)) == 'pdsurf_numba_tight_root' .or. &
            trim(adjustl(backend)) == 'pdsurf_numba_tight_root_fortran_pv') then
            pdsurf_engine = 'tight_root'
        endif
        inquire(unit=66, opened=log_open)
        write(*,'(A,A)') 'Depth kernel backend: ', trim(adjustl(backend))
        if (log_open) then
            write(66,'(A,A)') 'Depth kernel backend: ', trim(adjustl(backend))
            write(66,'(A,I0,A,I0,A,I0,A,I0)') 'Depth kernel grid/periods: nx=', nx, ' ny=', ny, &
                ' nz=', nz, ' kmax=', kmaxRc
            write(66,'(A,F10.4,A,F10.6)') 'Depth kernel minthk=', minthk, ' dln=', dlnVs
        endif
        if (env_status == 0 .and. (trim(adjustl(backend)) == 'pdsurf_numba' .or. &
            trim(adjustl(backend)) == 'pdsurf_numba_fortran_pv' .or. &
            trim(adjustl(backend)) == 'pdsurf_numba_tight_root' .or. &
            trim(adjustl(backend)) == 'pdsurf_numba_tight_root_fortran_pv')) then
            call depthkernel_pdsurf_numba(nx,ny,nz,vel,pvRc,sen_vsRc,sen_vpRc,sen_rhoRc, &
                 iwave,igr,kmaxRc,tRc,depz,minthk,dlnVs,pdsurf_engine)
            if (trim(adjustl(backend)) == 'pdsurf_numba_fortran_pv' .or. &
                trim(adjustl(backend)) == 'pdsurf_numba_tight_root_fortran_pv') then
!$omp parallel &
!$omp default(private) &
!$omp shared(depz,nx,ny,nz,minthk,kmaxRc,mmax,vel,tRc,pvRc,iwave,igr)
!$omp do collapse(2) schedule(dynamic)
                do jj=1,ny
                do ii=1,nx
                    vsz(1:nz)=vel(ii,jj,1:nz)
                    call depthkernel_column_phase_fd(mmax, vsz, kmaxRc, tRc, iwave, igr, &
                         depz, minthk, pv_col)
                    pvRc((jj-1)*nx+ii,1:kmaxRc)=pv_col(1:kmaxRc)
                enddo
                enddo
!$omp end do
!$omp end parallel
            ...
            dump_dir = ''
            call get_environment_variable('DAZI_DEPTHKERNEL_DUMP_DIR', dump_dir, status=env_status)
            if (log_open .and. env_status == 0) write(66,'(A,A)') 'Depth kernel dump dir: ', trim(adjustl(dump_dir))
            if (env_status == 0) call depthkernel_dump_arrays(nx,ny,nz,kmaxRc,pvRc,sen_vsRc, &
                 sen_vpRc,sen_rhoRc,trim(adjustl(dump_dir)))
            return
        endif

        ! Compute one complete 1D column per horizontal grid point. This is
        ! the same production interface exercised by depthkernel_lab.
!$omp parallel &
!$omp default(private) &
!$omp shared(depz,nx,ny,nz,minthk,dlnvs,dlnvp,dlnrho,kmaxRc,mmax,vel) &
!$omp shared(tRc,pvRc,sen_vpRc,sen_vsRc,sen_rhoRc,iwave,igr)
!$omp do collapse(2) schedule(dynamic)
        do jj=1,ny
    	do ii=1,nx
    	vsz(1:nz)=vel(ii,jj,1:nz)
        call depthkernel_column_fd(mmax, vsz, kmaxRc, tRc, iwave, igr, depz, minthk, &
             dlnVs, dlnVp, dlnrho, pv_col, col_sen_vs, col_sen_vp, col_sen_rho)
        pvRc((jj-1)*nx+ii,1:kmaxRc)=pv_col(1:kmaxRc)
        do i = 1, mmax
        do nn = 1,kmaxRc
            sen_vsRc((jj-1)*nx+ii,nn,i) = col_sen_vs(nn,i)
            sen_vpRc((jj-1)*nx+ii,nn,i) = col_sen_vp(nn,i)
            sen_rhoRc((jj-1)*nx+ii,nn,i) = col_sen_rho(nn,i)
        enddo
        enddo
        enddo
        enddo
	...
	!$omp end parallel
      ...
      subroutine depthkernel_dump_arrays(nx,ny,nz,kmaxRc,pvRc,sen_vsRc,sen_vpRc,sen_rhoRc,dump_dir)
        implicit none
        integer nx,ny,nz,kmaxRc,unit_id
        character(len=*) dump_dir
        real*8 pvRc(nx*ny,kmaxRc)
        real*8 sen_vpRc(nx*ny,kmaxRc,nz),sen_vsRc(nx*ny,kmaxRc,nz),sen_rhoRc(nx*ny,kmaxRc,nz)

        if (len_trim(dump_dir) == 0) return
        call system('mkdir -p ' // trim(adjustl(dump_dir)))
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/pv_depthkernel.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) pvRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/sen_vs_depthkernel.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) sen_vsRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/sen_vp_depthkernel.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) sen_vpRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/sen_rho_depthkernel.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) sen_rhoRc
        close(unit_id)
      end subroutine depthkernel_dump_arrays

      subroutine depthkernel_pdsurf_numba(nx,ny,nz,vel,pvRc,sen_vsRc,sen_vpRc,sen_rhoRc, &
                  iwave,igr,kmaxRc,tRc,depz,minthk,dln,pdsurf_engine_in)
        implicit none
        integer nx,ny,nz,kmaxRc,iwave,igr
        real vel(nx,ny,nz),depz(nz),minthk,dln
        real*8 tRc(kmaxRc)
        real*8 pvRc(nx*ny,kmaxRc)
        real*8 sen_vpRc(ny*nx,kmaxRc,nz),sen_vsRc(ny*nx,kmaxRc,nz),sen_rhoRc(ny*nx,kmaxRc,nz)

        character(len=1024) python_cmd, script_path, outdir, cmd, tile_env, env_value
        character(len=64) pdsurf_engine
        character(len=32) nx_s, ny_s, nz_s, minthk_s, iwave_s, igr_s, dln_s, tile_s
        character(len=*), intent(in) :: pdsurf_engine_in
        integer status, unit_id, tile_columns, cmd_status
        logical log_open, file_exists

        python_cmd = '/opt/miniconda3/envs/dispa/bin/python'
        script_path = '/Users/liuxin/Desktop/DSurf_test/DAzi_vorotomo_migration/depthkernel_lab/pdsurf_tile_senk_solver.py'
        outdir = '.dazi_senk_tile'
        tile_columns = 64
        pdsurf_engine = trim(adjustl(pdsurf_engine_in))
        env_value = ''
        call get_environment_variable('DAZI_PYTHON', env_value, status=status)
        if (status == 0) python_cmd = env_value
        env_value = ''
        call get_environment_variable('DAZI_PDSURF_SENK_SCRIPT', env_value, status=status)
        if (status == 0) script_path = env_value
        env_value = ''
        call get_environment_variable('DAZI_PDSURF_SENK_DIR', env_value, status=status)
        if (status == 0) outdir = env_value
        tile_env = ''
        call get_environment_variable('DAZI_PDSURF_TILE_COLUMNS', tile_env, status=status)
        if (status == 0) read(tile_env,*) tile_columns
        env_value = ''
        call get_environment_variable('DAZI_PDSURF_ENGINE', env_value, status=status)
        if (status == 0 .and. len_trim(env_value) > 0) pdsurf_engine = trim(adjustl(env_value))
        if (len_trim(pdsurf_engine) == 0) pdsurf_engine = 'default'

        call system('mkdir -p ' // trim(adjustl(outdir)))

        open(newunit=unit_id,file=trim(adjustl(outdir))//'/vel_senK.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) vel
        close(unit_id)
        ...
        cmd = trim(adjustl(python_cmd))//' '//trim(adjustl(script_path))// &
             ...
             ' --dln '//trim(adjustl(dln_s))// &
             ' --engine '//trim(adjustl(pdsurf_engine))// &
             ' --tile-columns '//trim(adjustl(tile_s))// &
             ' --outdir '//trim(adjustl(outdir))
        inquire(unit=66, opened=log_open)
        write(*,'(A,A)') 'Depth kernel pDSurf Python: ', trim(adjustl(python_cmd))
        write(*,'(A,A)') 'Depth kernel pDSurf script: ', trim(adjustl(script_path))
        write(*,'(A,A)') 'Depth kernel pDSurf outdir: ', trim(adjustl(outdir))
        write(*,'(A,A)') 'Depth kernel pDSurf engine: ', trim(adjustl(pdsurf_engine))
        write(*,'(A,I0)') 'Depth kernel pDSurf tile columns: ', tile_columns
        write(*,'(A,A)') 'Depth kernel backend command: ', trim(cmd)
        if (log_open) then
            write(66,'(A,A)') 'Depth kernel pDSurf Python: ', trim(adjustl(python_cmd))
            write(66,'(A,A)') 'Depth kernel pDSurf script: ', trim(adjustl(script_path))
            write(66,'(A,A)') 'Depth kernel pDSurf outdir: ', trim(adjustl(outdir))
            write(66,'(A,A)') 'Depth kernel pDSurf engine: ', trim(adjustl(pdsurf_engine))
            write(66,'(A,I0)') 'Depth kernel pDSurf tile columns: ', tile_columns
            write(66,'(A,A)') 'Depth kernel backend command: ', trim(cmd)
        endif
        call execute_command_line(trim(cmd), exitstat=cmd_status)
        if (cmd_status /= 0) then
            write(*,'(A,I0)') 'ERROR: depth kernel backend command failed with exit status ', cmd_status
            if (log_open) write(66,'(A,I0)') 'ERROR: depth kernel backend command failed with exit status ', cmd_status
            stop 9101
        endif

        inquire(file=trim(adjustl(outdir))//'/pv_pdsurf_numba.bin', exist=file_exists)
        if (.not. file_exists) then
            write(*,'(A)') 'ERROR: missing pDSurf output pv_pdsurf_numba.bin'
            if (log_open) write(66,'(A)') 'ERROR: missing pDSurf output pv_pdsurf_numba.bin'
            stop 9102
        endif
        inquire(file=trim(adjustl(outdir))//'/sen_vs_pdsurf_numba.bin', exist=file_exists)
        if (.not. file_exists) then
            write(*,'(A)') 'ERROR: missing pDSurf output sen_vs_pdsurf_numba.bin'
            if (log_open) write(66,'(A)') 'ERROR: missing pDSurf output sen_vs_pdsurf_numba.bin'
            stop 9103
        endif
        inquire(file=trim(adjustl(outdir))//'/sen_vp_pdsurf_numba.bin', exist=file_exists)
        if (.not. file_exists) then
            write(*,'(A)') 'ERROR: missing pDSurf output sen_vp_pdsurf_numba.bin'
            if (log_open) write(66,'(A)') 'ERROR: missing pDSurf output sen_vp_pdsurf_numba.bin'
            stop 9104
        endif
        inquire(file=trim(adjustl(outdir))//'/sen_rho_pdsurf_numba.bin', exist=file_exists)
        if (.not. file_exists) then
            write(*,'(A)') 'ERROR: missing pDSurf output sen_rho_pdsurf_numba.bin'
            if (log_open) write(66,'(A)') 'ERROR: missing pDSurf output sen_rho_pdsurf_numba.bin'
            stop 9105
        endif

        open(newunit=unit_id,file=trim(adjustl(outdir))//'/pv_pdsurf_numba.bin', &
             form='unformatted',access='stream',status='old',action='read')
        read(unit_id) pvRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(outdir))//'/sen_vs_pdsurf_numba.bin', &
             form='unformatted',access='stream',status='old',action='read')
        read(unit_id) sen_vsRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(outdir))//'/sen_vp_pdsurf_numba.bin', &
             form='unformatted',access='stream',status='old',action='read')
        read(unit_id) sen_vpRc
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(outdir))//'/sen_rho_pdsurf_numba.bin', &
             form='unformatted',access='stream',status='old',action='read')
        read(unit_id) sen_rhoRc
        close(unit_id)
      end subroutine depthkernel_pdsurf_numba
...
subroutine CalSurfG(nx,ny,nz,nparpi,vels,iw,rw,col,dsurf, &
              ...
              scxf,sczf,rcxf,rczf,nrc1,nsrcsurf1,kmax,nsrcsurf,nrcf, nar)
USE globalp
USE traveltime
IMPLICIT NONE
...
!       real*8 sen_rhoLg(nx*ny,kmaxLg,nz)
	...
	real cbst1
      ...
      real,parameter::ftol=1e-4
	      integer writepath
        character(len=1024) g_dump_dir
        integer env_status
	! Added by C.M.
	        integer dall
       REAL GVs(dall,nparpi)
       integer rbIndex
...
if(rbIndex.GE.1)then
    ...
    ! write(6,*)'to prevent this from occurring.'
        ...
        g_dump_dir = ''
        call get_environment_variable('DAZI_G_TRIPLET_DUMP_DIR', g_dump_dir, status=env_status)
        if (env_status == 0) call dump_g_triplets(nar, rw, iw, col, trim(adjustl(g_dump_dir)))
	deallocate(fdm)
	deallocate(velv,veln,ttn,nsts,btg)
	END subroutine

      subroutine dump_g_triplets(nar, rw, iw, col, dump_dir)
        implicit none
        integer(kind=8) nar
        real rw(*)
        integer iw(*), col(*)
        ...
        if (len_trim(dump_dir) == 0) return
        call system('mkdir -p ' // trim(adjustl(dump_dir)))
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/rw_triplets.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) rw(1:nar)
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/iw_triplets.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) iw(1:nar)
        close(unit_id)
        open(newunit=unit_id,file=trim(adjustl(dump_dir))//'/col_triplets.bin', &
             form='unformatted',access='stream',status='replace')
        write(unit_id) col(1:nar)
        close(unit_id)
      end subroutine dump_g_triplets
