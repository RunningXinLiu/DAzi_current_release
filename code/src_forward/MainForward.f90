...
program SurfAnisoForward
    use coord_utils, only: coord_mode_default, coord_mode_name, coord_mode_is_implemented, &
        latdeg_to_colatrad, londeg_to_lonrad, colatrad_to_latdeg, &
        lonrad_to_londeg
    use omp_lib
    implicit none
    ...
    integer msurf
    real,dimension(:),allocatable:: obst,dsyn,dist,periodRre
    ...
    read(10,*) writepath
    read(10,*) kmaxRc
    coord_mode = coord_mode_default
    if (.not. coord_mode_is_implemented(coord_mode)) stop 'unsupported coord_mode for forward modeling'
    ...
    write(66,*) 'coordinate mode'
    write(66,'(a)') trim(coord_mode_name(coord_mode))
    ...
    if(err.eq.0) then
        if(line(1:1).eq.'#') then
            ...
            istep1=0
            sta1_lat=latdeg_to_colatrad(sta1_lat)
            sta1_lon=londeg_to_lonrad(sta1_lon)
            ...
            sta2_lat=latdeg_to_colatrad(sta2_lat)
            sta2_lon=londeg_to_lonrad(sta2_lon)
    ...
    maxnar = int(real(spfra,kind=8) * real(dall,kind=8) * real(nx,kind=8) * &
        real(ny,kind=8) * real(nz,kind=8) * 3.0_8, kind=8) ! sparsity fraction
    if (maxnar < 1_8) stop 'invalid maxnar, check sparsefrac and data count'
    if (maxnar > int(huge(0),kind=8)) stop 'maxnar exceeds default integer range in forward code'
    ...
    if(checkstat > 0) stop 'dsyn allocation failed'
    ...
    do knum=1,kmax
        do srcnum=1,nsrc1(knum)
            ...
            sta1_latD=colatrad_to_latdeg(sta1_lat)
            sta1_lonD=lonrad_to_londeg(sta1_lon)
            ...
            do istep=1,nrc1(srcnum,knum)
                ...
                call delsph(sta1_lat,sta1_lon,sta2_lat,sta2_lon,dist1)
                sta2_lat=colatrad_to_latdeg(sta2_lat)
                sta2_lon=lonrad_to_londeg(sta2_lon)
                count1=count1+1
                Tvalue=obst(count1)
